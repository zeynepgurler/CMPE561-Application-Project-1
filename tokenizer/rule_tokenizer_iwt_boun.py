...
class RuleBasedUnifiedTokenizer:
    ...
    ama domain parametresi ile ITU Web Treebank'i de destekler.

    domain="boun":
      - Noktalama işaretlerini ayrı token yapar.
      - Sayı içindeki '.' ve ',' karakterlerine dokunmaz (3.14, 1,25 gibi).
      - Bazı kopula / zaman eklerini heuristik olarak ayırır:
          * ağlayacaktı -> ağlayacak + tı
          * şöyleydi    -> şöyle + ydi
          * girebilirdik -> girebilir + dik
          * böyledir    -> böyle + dir (fiil değilse)
      - Apostroflu biçimleri (SASA'nın, Meclis'in, Türkiye'ye) TEK token
        olarak bırakır.
      - Dışarıdan verilen MWE listeleriyle, token dizisi üzerinde TAM
        eşleşen çok sözcüklü ifadeleri tek token hâline getirir.

    domain="iwt" (ITU Web Treebank):
      - @mention[@user], @smiley[:)], @vocative[ahaha], @hashtag[#tag],
        @keyword[RT] gibi pattern'leri TEK token olarak korur.
      - Bu pattern'lerin içindeki [, ], :, ', ( gibi karakterler
        asla ayrı token yapılmaz.
      - Morfolojik split YAPMAZ.
      - MWE merge (Wikipedia/proper-name MWE) KAPALI (Emre Aydın, fotoğraf
        çekmek, bir kere vs. birleşmez).
    """
    ...
    def __init__(
        ...
        """
        mwe_path        : Satır başına bir MWE olacak şekilde text dosya yolu.
                          Örn: "ya da", "geri döndükten", "paldür küldür" ...
        proper_mwe_path: Satır başına çok kelimeli özel isim olacak şekilde
                          text dosya yolu.
                          Örn: "Ahmet Hamdi Tanpınar", "Beş Şehir", ...
        """
